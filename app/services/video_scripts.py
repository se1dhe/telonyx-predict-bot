...
def render_video_script_message(prediction: Prediction, index: int = 1, total: int = 1) -> str:
    ...
    return "\n".join(
        [
            f"🎬 <b>Сценарий короткого видео {index}/{total}</b>",
            f"⚽️ <b>{html_escape(match_title)}</b>",
            "",
            f"🎯 <b>Главная идея:</b> нейросеть против букмекеров: быстро показать, почему ставка {html_escape(main_bet)} выглядит логичной, а полный разбор увести в Телеграм.",
            "",
            ...
            "• Если есть нарезка моментов команд, ставь ее под текст; если нет — строгий фон в стиле нейросети и футбольной аналитики.",
            "",
            ...
            f"2. 3-10 сек: показать источник данных: футбольная статистика плюс линия {html_escape(bookmaker)}; вывести уверенность {confidence}/100.",
            ...
            f"5. 35-45 сек: призыв к действию: «Полный разбор, рискованный вариант{html_escape(odds_text)} и ссылка на линию уже в Телеграме».",
            "",
            "<b>Текст для озвучки:</b>",
            f"«Нейросеть проанализировала матч {html_escape(match_title)} через футбольную статистику и линию {html_escape(bookmaker)}. Уверенность модели — {confidence} из 100. Основная ставка — {html_escape(main_bet)}. Почему? {html_escape(reasoning)} Поэтому я не играю исход вслепую, а беру рынок, который статистически выглядит чище. Рискованный вариант{html_escape(odds_text)} и полный разбор уже загрузил в Телеграм. Ссылка в профиле».",
            "",
            "<b>Текст на экране:</b>",
            f"• {html_escape(match_title)}",
            f"• Уверенность нейросети: {confidence}/100",
            f"• Основная ставка: {html_escape(main_bet)}",
            f"• Почему: {html_escape(why)}",
            "• Полный разбор в Телеграме",
            "",
            f"<b>Справка:</b> {html_escape(league or 'турнир не указан')} | {html_escape(time_text)} | рискованный вариант: {html_escape(risky_bet)}",
        ]
    )
